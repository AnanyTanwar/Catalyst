...
//   3. simd_add_sub_weights  — accumulator += (col_add - col_sub)
//   4. simd_screlu_forward   — SCReLU forward pass
//      output += clamp(acc[i], 0, QA)^2 * weight[i]  (int16 -> int32)
...
//
// HIDDEN_SIZE must be a multiple of the widest register used.
// 768 satisfies all tiers (multiple of 32, 16, 8).
// ============================================================

#include "intrinsics.h" // FORCE_INLINE
#include <cstdint>
...
// ============================================================
// Constants (must match nnue.h QA)
// Updated to 181 for Midnight netM006.nnue compatibility.
// ============================================================
inline constexpr int16_t SCRELU_MIN = 0;
inline constexpr int16_t SCRELU_MAX = 181; // QA — must match nnue.h
...
FORCE_INLINE void simd_add_weights(int16_t *__restrict__ acc,
                                   const int16_t *__restrict__ col) {
  ...
  for (int i = 0; i < ITERS; ++i) {
    __m512i a =
        _mm512_load_si512(reinterpret_cast<const __m512i *>(acc + i * 32));
    __m512i b =
        _mm512_load_si512(reinterpret_cast<const __m512i *>(col + i * 32));
  ...
  for (int i = 0; i < ITERS; ++i) {
    ...
    __m256i b =
        _mm256_load_si256(reinterpret_cast<const __m256i *>(col + i * 16));
...
template <int HIDDEN_SIZE>
FORCE_INLINE void simd_sub_weights(int16_t *__restrict__ acc,
                                   const int16_t *__restrict__ col) {
#if defined(SIMD_AVX512VNNI) || defined(SIMD_AVX512)
  static_assert(HIDDEN_SIZE % 32 == 0, "HIDDEN_SIZE must be multiple of 32");
  constexpr int ITERS = HIDDEN_SIZE / 32;
...
// ============================================================
// 3.  simd_add_sub_weights  (combined move: remove from + add to)
...
FORCE_INLINE void simd_add_sub_weights(int16_t *__restrict__ acc,
  ...
  for (int i = 0; i < ITERS; ++i) {
    __m512i a =
        _mm512_load_si512(reinterpret_cast<const __m512i *>(acc + i * 32));
    __m512i add =
        _mm512_load_si512(reinterpret_cast<const __m512i *>(col_add + i * 32));
    __m512i sub =
        _mm512_load_si512(reinterpret_cast<const __m512i *>(col_sub + i * 32));
    _mm512_store_si512(reinterpret_cast<__m512i *>(acc + i * 32),
  ...
  for (int i = 0; i < ITERS; ++i) {
    __m256i a =
        _mm256_load_si256(reinterpret_cast<const __m256i *>(acc + i * 16));
    __m256i add =
        _mm256_load_si256(reinterpret_cast<const __m256i *>(col_add + i * 16));
    __m256i sub =
        _mm256_load_si256(reinterpret_cast<const __m256i *>(col_sub + i * 16));
    _mm256_store_si256(reinterpret_cast<__m256i *>(acc + i * 16),
  ...
  for (int i = 0; i < ITERS; ++i) {
    __m128i a = _mm_load_si128(reinterpret_cast<const __m128i *>(acc + i * 8));
    __m128i add =
        _mm_load_si128(reinterpret_cast<const __m128i *>(col_add + i * 8));
    __m128i sub =
        _mm_load_si128(reinterpret_cast<const __m128i *>(col_sub + i * 8));
    _mm_store_si128(reinterpret_cast<__m128i *>(acc + i * 8),
...
//
//     Computes one half of the SCReLU forward pass:
//       sum += clamp(acc[i], 0, QA)^2 * weights[i]
//     for i in [0, HIDDEN_SIZE).
//
//     Returns the int32_t dot product for one accumulator half.
//     Call twice — once for white_acc (weights offset 0),
//     once for black_acc (weights offset HIDDEN_SIZE).
//
//     QA = 181 (Midnight). Max value before squaring: 181^2 = 32761,
//     which fits in int16. Max after * weight (int16): 32761 * 32767
//     = ~1.07e9, fits in int32. Widening to int32 before accumulating
//     is still required to avoid overflow across HIDDEN_SIZE=768 terms.
// ============================================================
...
FORCE_INLINE int32_t simd_screlu_forward(const int16_t *__restrict__ acc,
  ...
  for (int i = 0; i < ITERS; ++i) {
    __m512i x =
        _mm512_load_si512(reinterpret_cast<const __m512i *>(acc + i * 32));
    __m512i w =
        _mm512_load_si512(reinterpret_cast<const __m512i *>(weights + i * 32));
    ...
    __m256i r_ll =
        _mm256_mullo_epi32(_mm256_mullo_epi32(x32_ll, x32_ll), w32_ll);
    __m256i r_lh =
        _mm256_mullo_epi32(_mm256_mullo_epi32(x32_lh, x32_lh), w32_lh);
    __m256i r_hl =
        _mm256_mullo_epi32(_mm256_mullo_epi32(x32_hl, x32_hl), w32_hl);
    __m256i r_hh =
        _mm256_mullo_epi32(_mm256_mullo_epi32(x32_hh, x32_hh), w32_hh);
  ...
  for (int i = 0; i < ITERS; ++i) {
    ...
    auto accum256 = [&](__m256i xv, __m256i wv) -> __m512i {
      ...
      __m256i r_lo =
          _mm256_mullo_epi32(_mm256_mullo_epi32(x32_lo, x32_lo), w32_lo);
      __m256i r_hi =
          _mm256_mullo_epi32(_mm256_mullo_epi32(x32_hi, x32_hi), w32_hi);
  ...
  for (int i = 0; i < ITERS; ++i) {
    __m256i x =
        _mm256_load_si256(reinterpret_cast<const __m256i *>(acc + i * 16));
    __m256i w =
        _mm256_load_si256(reinterpret_cast<const __m256i *>(weights + i * 16));
  ...
  for (int i = 0; i < ITERS; ++i) {
    ...
    __m128i w =
        _mm_load_si128(reinterpret_cast<const __m128i *>(weights + i * 8));
...
FORCE_INLINE void simd_init_accumulator(int16_t *__restrict__ acc,
  ...
  for (int i = 0; i < ITERS; ++i)
    _mm512_store_si512(
        reinterpret_cast<__m512i *>(acc + i * 32),
  ...
  for (int i = 0; i < ITERS; ++i)
    _mm256_store_si256(
        reinterpret_cast<__m256i *>(acc + i * 16),
  ...
  for (int i = 0; i < ITERS; ++i)
    _mm_store_si128(
        reinterpret_cast<__m128i *>(acc + i * 8),
